...
type PolicyCache struct {
	mu      sync.RWMutex
	active  map[string]*PolicyContext
	baseDir string
	lkgDir  string
}
...
func NewPolicyCache(baseDir, lkgDir string) *PolicyCache {
	...
	return &PolicyCache{
		active:  make(map[string]*PolicyContext),
		baseDir: baseDir,
		lkgDir:  lkgDir,
	}
}

func (c *PolicyCache) Activate(ctx *PolicyContext) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *PolicyCache) GetActive(dataStoreID string) (*PolicyContext, bool) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *PolicyCache) GetLastKnownGood(dataStoreID string) (*PolicyContext, error) {
...
func (c *PolicyCache) persist(path string, ctx *PolicyContext) error {
	tmp := path + ".tmp"
